...
const OTPField = <T extends ValidComponent = 'div'>(
  ...
  const [local, others] = splitProps(props as OTPFieldProps, [
    'class',
    'error',
  ]);
  return (
    <OtpField
      class={cn(
        'group/input flex items-center gap-2 disabled:cursor-not-allowed has-[:disabled]:opacity-50',
        local.class,
        local.error ? 'error' : ''
      )}
      {...others}
...
const OTPFieldSlot: Component<ComponentProps<'div'> & { index: number }> = (
  ...
  return (
    <div
      class={cn(
        'group relative flex size-10 items-center justify-center border-y border-r group-[.error]/input:border-destructive text-sm first:rounded-l-md first:border-l last:rounded-r-md',
        local.class
      )}
      {...others}
    >
